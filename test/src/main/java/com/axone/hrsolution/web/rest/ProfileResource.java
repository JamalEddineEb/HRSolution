...
/**
 * REST controller for managing {@link com.axone.hrsolution.domain.Profile}.
 */
@RestController
...
public class ProfileResource {
    ...
    public List<Profile> getAllProfiles(
        ...
        if ("recruiter-is-null".equals(filter)) {
            log.debug("REST request to get all Profiles where recruiter is null");
            return StreamSupport.stream(profileRepository.findAll().spliterator(), false)
                .filter(profile -> profile.getRecruiter() == null)
                .toList();
        }

        if ("employer-is-null".equals(filter)) {
            log.debug("REST request to get all Profiles where employer is null");
            return StreamSupport.stream(profileRepository.findAll().spliterator(), false)
                .filter(profile -> profile.getEmployer() == null)
                .toList();
        }

        if ("admin-is-null".equals(filter)) {
            log.debug("REST request to get all Profiles where admin is null");
            return StreamSupport.stream(profileRepository.findAll().spliterator(), false)
                .filter(profile -> profile.getAdmin() == null)
                .toList();
        }

        if ("account-is-null".equals(filter)) {
            log.debug("REST request to get all Profiles where account is null");
            return StreamSupport.stream(profileRepository.findAll().spliterator(), false)
