...
export class RecruiterComponent implements OnInit {
  ...
  trackId = (_index: number, item: IRecruiter): number => this.recruiterService.getRecruiterIdentifier(item);
  ...
  ngOnInit(): void {
    this.subscription = combineLatest([this.activatedRoute.queryParamMap, this.activatedRoute.data])
      .pipe(
        ...
        tap(() => {
          if (!this.recruiters || this.recruiters.length === 0) {
            this.load();
          }
        }),
      )
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.recruiters = this.refineData(dataFromBody);
  }

  protected refineData(data: IRecruiter[]): IRecruiter[] {
    const { predicate, order } = this.sortState();
    return predicate && order ? data.sort(this.sortService.startSort({ predicate, order })) : data;
  }

  protected fillComponentAttributesFromResponseBody(data: IRecruiter[] | null): IRecruiter[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.recruiterService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
