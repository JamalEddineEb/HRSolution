...
describe('Employer Management Detail Component', () => {
  ...
  describe('OnInit', () => {
    it('Should load employer on init', async () => {
      const harness = await RouterTestingHarness.create();
      const instance = await harness.navigateByUrl('/', EmployerDetailComponent);

      // THEN
      expect(instance.employer()).toEqual(expect.objectContaining({ id: 123 }));
    });
  });
