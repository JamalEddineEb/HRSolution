...
/**
 * REST controller for managing {@link com.axone.hrsolution.domain.ApplicationUser}.
 */
@RestController
...
public class ApplicationUserResource {
        ...
        Optional<ApplicationUser> result = applicationUserRepository
            ...
            .map(existingApplicationUser -> {
                if (applicationUser.getProfileImage() != null) {
                    existingApplicationUser.setProfileImage(applicationUser.getProfileImage());
                }
                if (applicationUser.getProfileImageContentType() != null) {
                    existingApplicationUser.setProfileImageContentType(applicationUser.getProfileImageContentType());
                }
    ...
    public List<ApplicationUser> getAllApplicationUsers(
        ...
        if ("account-is-null".equals(filter)) {
            log.debug("REST request to get all ApplicationUsers where account is null");
            return StreamSupport.stream(applicationUserRepository.findAll().spliterator(), false)
                .filter(applicationUser -> applicationUser.getAccount() == null)
                .toList();
        }
